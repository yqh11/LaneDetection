...
nx = 9
ny = 6
file_paths = glob.glob("./camera_cal/calibration*.jpg")  # glob.glob返回所有匹配的文件路径列表
...
def cal_line_center(img):
    undistort_img = img_undistort(img,mtx,dist)                   # 矫正
    rigin_pipeline_img = pipeline(undistort_img)                  # 边缘提取
    trasform_img = img_perspect_transform(rigin_pipeline_img,M)   # 透视变换
...
def process_image(img):
    ...
    cv2.imwrite("rigin_pipline_img.jpg",rigin_pipline_img)
    # 透视变换
    transform_img = img_perspect_transform(rigin_pipline_img,M)
    cv2.imwrite("transform_img.jpg", transform_img)
    # 拟合车道线精准定位
    ...
    cv2.imwrite("transform_img_inv.jpg", transform_img_inv)
    # 曲率和偏离距离
