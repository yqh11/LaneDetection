...
def get_weight_matrix(imA, imB):
    ...
    # OpenCV-contrib有一个名为FastLineDetector的东西，如果它被用作LSD的替代品似乎很好
...
def main(save_weights=False):
    ...
    back = back[::-1, ::-1, :]  #img[:,::-1,:] 水平翻转    img[::-1,:,:] 上下翻转
    left = cv2.transpose(left)[::-1]  #转置后 上下翻转
    right = np.flip(cv2.transpose(right), 1)   # 左右翻转，意味着把列看成整体，列的顺序发生颠倒，每一列的元素不发生改变
    cv2.imwrite("right_flip.jpg", right)
    cv2.imwrite("back_flip.jpg", back)
    cv2.imwrite("left_flip.jpg", left)
    ...
    # 将G作为权值可得融合后的图像为front * G + (1 - G) * left。
    ...
    cv2.imwrite("ch1.jpg", ch1)
    cv2.imwrite("ch2.jpg", ch2)
    cv2.imwrite("ch3.jpg", ch3)
